...
> **STATUS 30 AUG 2026 — SPRINT SHIPPED.** Everything in scope was implemented on branch `cro-sprint` and merged to main: checkout fix, welcome offer + email capture (Resend domain verified, VEYLO10 live), 2-col mobile shop with anchor-first merchandising, sticky add-to-bag, cart drawer progress/cross-sell/bundle hint, payment logos, webhook hardening. Two post-merge changes supersede parts of this brief: **(1) checkout is now Stripe-HOSTED** (`/checkout` redirects to checkout.stripe.com; the embedded iframe mis-sized itself in mobile in-app browsers no matter the container, so P0-B/P0-C container work is moot — do not resurrect the embed) with Adaptive Pricing disabled and locale pinned en-GB; **(2) Meta CAPI is LIVE** via a long-lived user token (expires 29 Oct 2026, renewal reminder scheduled) — P0-A item 5 and P0-C are resolved. Delivery copy rule stands: dispatch-speed framing only, no transit times on selling surfaces. **Remaining backlog for a future sprint:** honest-reviews pipeline (P2-5), abandoned-checkout recovery + order-confirmation emails (P2-8, webhook already receives `checkout.session.expired`), announcement-bar rotation, collection page pass.
